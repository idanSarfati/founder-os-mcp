...
# The "Constitution" of Founder OS
...
def install_rules():
    """Writes the .cursorrules file to the project root."""
    # Determine the project root (assuming this script is in /config)
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(current_dir)
    target_path = os.path.join(project_root, ".cursorrules")

    print(f"[SEARCH] Checking for existing rules at: {target_path}")

    if os.path.exists(target_path):
        print("[INFO] Governance Rules (.cursorrules) already exist. Skipped to prevent overwrite.")
        return

    try:
        with open(target_path, "w", encoding="utf-8") as f:
            f.write(GOVERNANCE_RULES)
        print("[OK] Governance Rules installed successfully.")
    except Exception as e:
        print(f"[ERROR] Error installing rules: {e}")
